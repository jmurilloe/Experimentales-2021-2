clear all; close all; 
t0 = 0.0; tf = 100; N = 500; w2 = 0.1; b = 0.05;
dt = (tf-t0)/N; t(1) = t0; v(1) = 0; x(1) = 10;
% d^{2}x/dt^{2} = - w_{0}^{2}*x
f = @(t,x,v) v;     %dx/dt = v
g = @(t,x,v) -w2*x - b*v; %dv/dt = - w2*x
for n=1:N
  t(n+1) = t(n)+dt;
...
title('MAS, x&t','fontsize',14); xlabel('t(seg)');ylabel('x(m)');
figure (2); clf; 
plot(t,v,'b','linewidth',2); grid on;
title('MAS, v&t','fontsize',14); xlabel('t(seg)');ylabel('v(m/seg)');
figure (3); clf; 
plot(x,v,'b','linewidth',2); grid on;
title('MAS, x&v','fontsize',14); xlabel('x(m)');ylabel('v(m/seg)');